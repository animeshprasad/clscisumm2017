...
tr_texts_r, tr_texts_c, tr_labels = read_data.get_data('1b')
d_texts_r, d_texts_c = read_data.get_data('test1b')

print('Found {} samples with label.'.format(len(tr_texts_c)))
...
tokenizer.fit_on_texts(tr_texts_r + tr_texts_c + d_texts_r+ d_texts_c)
...
model.compile(loss='categorical_crossentropy',optimizer=optimizer=SGD(lr=0.0001,clipvalue=0.99, decay=1e-6, momentum=0.9, nesterov=True))

model.fit([x1_train,x2_train], y_train, nb_epoch=3,
          validation_data=([x1_val, x2_val], y_val),class_weight=None)
