...
indices = np.arange(x1_val.shape[0])
np.random.shuffle(indices)
x1_val = x1_val[indices]
...
model.compile(loss='categorical_crossentropy',optimizer=RMSprop(lr=0.0001,clipvalue=0.99))

model.fit([x1_train,x2_train], y_train, nb_epoch=5,
          validation_data=([x1_val, x2_val], y_val),class_weight=class_weight, callbacks=callbacks)
...
print ('Predict')
k=model.predict([x1_test, x2_test,]).argmax(1)
print( k)
print ('True')
l=y_test.argmax(1)
print( l )
