...
from keras.layers import Dense, Input, Flatten, GRU, Dropout, Activation, TimeDistributed
from keras.layers import Conv1D, MaxPooling1D, Embedding, Bidirectional, Merge, GlobalMaxPooling1D, RepeatVector
...
left.add(Bidirectional(GRU(output_dim=hidden_dims, input_shape=(MAX_SEQUENCE_LENGTH,EMBEDDING_DIM),return_sequences=True)))
#left.add(TimeDistributed(Dense(100)))

right = Sequential()
right.add(embedding_layer1_2)
right.add(Bidirectional(GRU(output_dim=hidden_dims, input_shape=(MAX_SEQUENCE_LENGTH,EMBEDDING_DIM),return_sequences=True)))


mid = Sequential()
mid.add(embedding_layer2)
mid.add(Bidirectional(GRU(output_dim=hidden_dims, input_shape=(MAX_SEQUENCE_LENGTH,EMBEDDING_DIM),return_sequences=True)))
mid.add(Lambda(lambda x: K.sum(x, axis=1), output_shape=(2*hidden_dims,)))
...
middle.add(Dense(2*hidden_dims))
middle.add(Dropout(0.2))
middle.add(Activation('sigmoid'))
middle.add(RepeatVector(MAX_SEQUENCE_LENGTH))

model1 = Sequential()
model1.add(Merge([left, middle], mode='mul'))
model1.add(Lambda(lambda x: K.sum(x, axis=1), output_shape=(2*hidden_dims,)))
#model1.add(Dropout(0.2))

model2 = Sequential()
model2.add(Merge([middle, right], mode='mul'))
model2.add(Lambda(lambda x: K.sum(x, axis=1), output_shape=(2*hidden_dims,)))
...
model=Sequential()
model.add(Merge([modelA, mid], mode='concat'))


model.add(Dense(2))
model.add(Activation('softmax'))
model.compile(loss='categorical_crossentropy',optimizer=RMSprop(lr=0.0001,clipvalue=0.99, decay=1e-6))
callbacks = [EarlyStopping(monitor='val_loss', patience=0)]
model.fit([x1_1_train,x2_train,x1_2_train], y_train, nb_epoch=2,
