...
class tfidf:
  ...
  def similarities(self, list_of_words, doc_subset):
    ...
    for doc in self.documents:
      ...
      if len(doc_dict.keys()) < 5:
          sims[doc[0]] = 0
          continue
      score += 0.001*max_counts
      #score += max_counts/len(doc_dict.keys())
      #print max_counts
        ...
        if '-' in file:
            ...
            for c in Counter(300):
                try:
                    ...
                    continue
                document[file + ''.join(str(index) for index in c)] = [stemmer.stem(plural) for plural in flatten(doc)]
...
if __name__ == '__main__':
    ...
    docs = read_training_files("../data/Test-Set-2017", 'tdata')
    for filenames in docs:

        docs_r = parsexml_lines("../data/Test-Set-2017", filenames)
        table = tfidf()
        for name in docs_r:
        ...
        for lines in docs[filenames]:
                lines=[stemmer.stem(plural) for plural in flatten(lines)]
                print lines
