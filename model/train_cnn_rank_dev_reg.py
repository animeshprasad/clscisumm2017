...
#class_weight[1]= class_weight[1]+ 1
...
model.compile(loss='binary_crossentropy',optimizer=SGD(lr=0.0001,clipvalue=0.99, decay=1e-6, momentum=0.9, nesterov=True))
...
print ('Predict')
k=model.predict([x1_1_test, x2_test,x1_2_test])
print( k)
print ('True')
l=y_test
print( l )

#print( np.sum((k-l)*(k-l))/len(k))
a,b,c,d=precision_recall_fscore_support(np.ceil(k),l)
print (a,  b, c)
print (np.dot(c,d)/np.sum(d))


to_test=model.predict([dt_data1_1, dt_data2,dt_data1_2])
print('Getting Ranks')
