...
embeddings_index = {}
f = open('/Users/animeshprasad/PycharmProjects/glove/glove.6B.300d.txt')
for line in f:
    values = line.split()
    word = values[0]
...
tokenizer.fit_on_texts(tr_texts_r1 + tr_texts_r2 + tr_texts_c + d_texts_r1 + d_texts_r2 +\
                         d_texts_c + t_texts_r1 + t_texts_r2 + t_texts_c)
...
t_data1_1 = pad_sequences(t_sequences1_1, maxlen=MAX_SEQUENCE_LENGTH, padding='post', truncating='post')
t_data1_2 = pad_sequences(t_sequences1_2, maxlen=MAX_SEQUENCE_LENGTH, padding='post', truncating='post')
t_data2 = pad_sequences(t_sequences2, maxlen=2*MAX_SEQUENCE_LENGTH, padding='post', truncating='post')
...
try:
    while True:
        ...
        #print(a)
        #raw_input()
        print(tokens[counter-1][2],tokens[counter-1][3],a.index(max(a)))
