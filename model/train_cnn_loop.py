...
#t_texts_r1, t_texts_r2, t_texts_c, t_line_no = read_data_downsampled_rank.get_data('1t')
...
tokenizer.fit_on_texts(tr_texts_r1 + tr_texts_r2 + tr_texts_c )#+ t_texts_r1 + t_texts_r2 + t_texts_c)
...
#t_sequences1_1 = tokenizer.texts_to_sequences(t_texts_r1)
#t_sequences1_2 = tokenizer.texts_to_sequences(t_texts_r2)
...
while True:
    model.compile(loss='categorical_crossentropy',optimizer=SGD(lr=0.0001,clipvalue=0.99, decay=1e-6, momentum=0.9, nesterov=True))#Adam(clipvalue=0.99))

    model.fit([x1_1_train,x2_train,x1_2_train], y_train, nb_epoch=20,
          validation_data=([x1_1_val, x2_val, x1_2_val], y_val),class_weight=class_weight)
	
    scores = model.evaluate([x1_1_test, x2_test,x1_2_test], y_test, verbose=0)
    print ('Predict')
    print(  model.predict([x1_1_test, x2_test,x1_2_test]).argmax(1))
    print ('True')
    print(  y_test.argmax(1))

    a,b,c,d=precision_recall_fscore_support(model.predict([x1_1_test, x2_test,x1_2_test]).argmax(1), y_test.argmax(1))
    print (a,  b, c)
    print (np.dot(c,d)/np.sum(d))

    if np.dot(c,d)/np.sum(d) > 0.67:
		raw_input()
